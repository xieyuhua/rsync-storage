...
func main() {
	...
	// 创建Watcher实例
	w, err := core.NewWatcher(c, PutChan, DeleteChan)
	if err != nil {
	...
	// 创建CheckJob实例
	j := core.NewCheckJob(c, PutChan, s)
	// 异步处理定期对账任务
	go j.Run(ctx)

	// 异步处理变更事件
	t := core.NewTransfer(c, PutChan, DeleteChan, s)
	for i := 0; i < 8; i++ {
		go t.Run(ctx)
	}
